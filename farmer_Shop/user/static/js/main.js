...
(function ($) {
        ...
        ajaxCall: function (action) {
            ...
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    console.error('Connection error:', errorThrown);
                },
    ...
    var totalPriceVariant = function () {
        $(".tf-product-info-list,.tf-cart-item,.mini-cart-item").each(function () {
            ...
            function updateTotalPrice(price, scope) {
                var currentPrice = price || parseFloat(scope.find(".price-on-sale").text().replace(/[₹$,]/g, ""));
        ...
        function updateCartTotal() {
            ...
            if (typeof $ === 'undefined') {
                // Don't retry indefinitely - only try once
                console.log('jQuery not loaded, skipping cart total update');
                return;
            }
        ...
        // Run on document ready - only on product listing pages
        $(document).ready(function() {
            if (document.querySelector('.tf-cart-item') === null) {
                updateCartTotal();
            }
        });

        // Only run on product listing pages, not on cart page
        if (document.querySelector('.tf-cart-item') === null) {
            updateCartTotal();
        }

        // Let the remove link work normally (it will redirect to server)
  ...
  -------------------------------------------------------------------------*/
    ...
    var authModals = function () {
        // User icon click to open login modal
        $('.user-icon-trigger').on('click', function (e) {
            e.preventDefault();
        ...
        $('[href="#registerModal"]').on('click', function (e) {
            e.preventDefault();
        ...
        $('[href="#loginModal"]').on('click', function (e) {
            e.preventDefault();
